#!/usr/bin/env python3
"""Unified benchmark runner for SpMV."""
...
KERNELS_DIR = BENCHMARKS_DIR / "kernels" / "benchmark" / "spmv"
TACO_EXE = BENCHMARKS_DIR / "taco" / "build" / "spmv_taco"
DEFAULT_OUTPUT = BENCHMARKS_DIR / "results" / "csv" / "benchmark_spmv.csv"
DEFAULT_FAILED = BENCHMARKS_DIR / "results" / "csv" / "benchmark_spmv_failed.csv"


def write_dsl_file(path: Path, rows: int, cols: int, sparse_format: str) -> None:
    tensor_format = "CSR" if sparse_format == "csr" else "CSC"
    content = (
        f"tensor y : Dense<{rows}>;\n"
        f"tensor A : {tensor_format}<{rows}, {cols}>;\n"
        f"tensor x : Dense<{cols}>;\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run unified SpMV benchmark")
...
def main() -> int:
    args = parse_args()
    if args.max_matrices <= 0 or args.iterations <= 0 or args.trials <= 0:
        raise ValueError("--max-matrices, --iterations, and --trials must be > 0")
    ...
    if not selected:
        raise ValueError("No canonical matrices found for spmv")
    ...
    for fmt in formats:
        for matrix in selected:
            dsl_file = KERNELS_DIR / "_dsl_tmp" / f"{KERNEL}_{matrix.name}_{fmt}.tc"
            write_dsl_file(dsl_file, matrix.rows, matrix.cols, fmt)
    ...
    if results:
        save_csv(results, args.output, SpMVBenchmarkResult)
    if failures:
