#!/usr/bin/env python3
"""Unified benchmark runner for SpElMul."""
...
KERNELS_DIR = BENCHMARKS_DIR / "kernels" / "benchmark" / "spelmul"
TACO_EXE = BENCHMARKS_DIR / "taco" / "build" / "spelmul_taco"
DEFAULT_OUTPUT = BENCHMARKS_DIR / "results" / "csv" / "benchmark_spelmul.csv"
DEFAULT_FAILED = BENCHMARKS_DIR / "results" / "csv" / "benchmark_spelmul_failed.csv"
DEFAULT_PAIRS = BENCHMARKS_DIR / "results" / "csv" / "spelmul_pairs_used.csv"
...
def write_dsl_file(path: Path, rows: int, cols: int, sparse_format: str) -> None:
    ...
    content = (
        ...
        f"tensor B : {tensor_format}<{rows}, {cols}>;\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run unified SpElMul benchmark")
...
def main() -> int:
    ...
    if not pairs:
        raise ValueError("No compatible matrix pairs found for spelmul")
    ...
    for fmt in formats:
        for matrix_a, matrix_b in pairs:
            ...
            write_dsl_file(dsl_file, matrix_a.rows, matrix_a.cols, fmt)
