#!/usr/bin/env python3
"""Unified benchmark runner for SpMM."""
...
KERNELS_DIR = BENCHMARKS_DIR / "kernels" / "benchmark" / "spmm"
TACO_EXE = BENCHMARKS_DIR / "taco" / "build" / "spmm_taco"
DEFAULT_OUTPUT = BENCHMARKS_DIR / "results" / "csv" / "benchmark_spmm.csv"
DEFAULT_FAILED = BENCHMARKS_DIR / "results" / "csv" / "benchmark_spmm_failed.csv"


def write_dsl_file(path: Path, rows: int, cols: int, output_cols: int, sparse_format: str) -> None:
    tensor_format = "CSR" if sparse_format == "csr" else "CSC"
    content = (
        f"tensor C : Dense<{rows}, {output_cols}>;\n"
        f"tensor A : {tensor_format}<{rows}, {cols}>;\n"
        f"tensor B : Dense<{cols}, {output_cols}>;\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run unified SpMM benchmark")
...
def main() -> int:
    args = parse_args()
    if args.max_matrices <= 0 or args.iterations <= 0 or args.output_cols <= 0 or args.trials <= 0:
        raise ValueError("--max-matrices, --iterations, --output-cols, and --trials must be > 0")
    ...
    if not selected:
        raise ValueError("No canonical matrices found for spmm")
    ...
    for fmt in formats:
        for matrix in selected:
            dsl_file = KERNELS_DIR / "_dsl_tmp" / f"{KERNEL}_{matrix.name}_{fmt}_n{args.output_cols}.tc"
            write_dsl_file(dsl_file, matrix.rows, matrix.cols, args.output_cols, fmt)
            ...
            for config_spec in config_specs:
                try:
                    ...
                    metrics, trial_summary, selected_stdout, hwc_fields = run_trials(
                        [str(exe_file), str(matrix.path), str(args.iterations)],
                        trials=args.trials,
                        timeout=1200,
            ...
            if use_taco:
                try:
                    metrics, trial_summary, selected_stdout, hwc_fields = run_trials(
                        [str(TACO_EXE), str(matrix.path), str(args.iterations), fmt],
                        trials=args.trials,
                        timeout=1200,
                        hwc_config=hwc_config,
                    )
    ...
    if results:
        save_csv(results, args.output, SpMMBenchmarkResult)
    if failures:
